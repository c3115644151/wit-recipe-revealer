...
function RenderCardCrafting(r, card_y)
	...
	for ii = 1, ing_count do
		local ing = ings[ii]
		local hl = (ing.type == RR_NAME)
		MakeSlot(RR_CONTENT, ing.type, start_x + (ii - 1) * 58, card_y, ing.amount, hl)
	end
	MakeArrow(RR_CONTENT, start_x + ing_count * 58 - 10, card_y)
	MakeSlot(RR_CONTENT, r.product or r.name, start_x + ing_count * 58 + 32, card_y, nil, false)
	...
	if state ~= nil then
		...
		local craft_btn = RR_CONTENT:AddChild(ImageButton("images/crafting_menu.xml", "ingredient_craft.tex", "ingredient_craft.tex"))
		if craft_btn then
...
function RenderCardCooking(r, card_y)
	...
	local pri = RR_CONTENT:AddChild(Text(NEWFONT, 18))
	if pri then
	...
	local conds = FormatCookCondition(r, RR_NAME)
	if #conds > 0 then
		local ct = RR_CONTENT:AddChild(Text(NEWFONT, 20))
		if ct then
	...
	if r.card_def and r.card_def.ingredients then
		...
		if r.test then
			...
			for slot_i = 1, #ings_list do
				...
				if cur ~= nil and cur ~= RR_NAME then
					-- 检查背包是否已有足够此食材
					local bp_check = {}
					for _, v in ipairs(bp_items) do
						local name = RR_COOKING_ALIASES[v] or v
	...
	if #ings_list > 0 then
		...
		for ii = 1, 4 do
			local hl = (ings_list[ii] == RR_NAME)
			local need_amt = ings_list[ii] and need_map[ings_list[ii]] or nil
			MakeSlot(RR_CONTENT, ings_list[ii], slot_start_x + (ii - 1) * 58, card_y - 8, need_amt, hl, nil, nil, nil, false)
		end
		MakeArrow(RR_CONTENT, slot_start_x + 4 * 58 - 10, card_y - 8)
		MakeSlot(RR_CONTENT, r.name, slot_start_x + 4 * 58 + 32, card_y - 8, nil, false, nil, nil, nil, false)
...
function RenderCards(recipes, card_h, card_spacing, render_card_fn)
	if RR_CONTENT == nil then return end
	RR_CONTENT:KillAllChildren()

	local total = #recipes
	local pages = math.max(1, math.ceil(total / RR_PAGE_SIZE))
	if RR_PAGE > pages then RR_PAGE = pages end
	if RR_PAGE < 1 then RR_PAGE = 1 end

	if RR_PG_TEXT then RR_PG_TEXT:SetString(RR_PAGE .. " / " .. pages) end

	local start_i = (RR_PAGE - 1) * RR_PAGE_SIZE + 1
	local end_i = math.min(start_i + RR_PAGE_SIZE - 1, total)

	for idx = start_i, end_i do
		local r = recipes[idx]
		local local_i = idx - start_i
		local card_y = -local_i * card_spacing + 25
		local card_bg = RR_CONTENT:AddChild(Image("images/global.xml", "square.tex"))
